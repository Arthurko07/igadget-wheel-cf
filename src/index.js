...
/* Глобальные переменные: bindings одинаковы для всех запросов деплоя */
let ENV = null;
let APP_ORIGIN = '';
let bot = null;
...
async function apiSpin(request) {
  ...
  // Розыгрыш по процентам (если сумма != 100 — нормализуем пропорционально)
...
async function apiStats() {
  ...
  return json({
    totalSpins: total,
    byPrize,
    recent: recent.map(r => ({ username: r.username, prizeName: r.prize_name, at: r.at })),
  });
}
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      /* Подключение вебхука (один раз после первого деплоя) */
