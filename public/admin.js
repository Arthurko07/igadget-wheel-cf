/* ⚙️ iGadget Admin — вероятности в процентах */
...
function activeTotal() {
  return prizes.filter(p => p.active).reduce((s, p) => s + (Number(p.percent) || 0), 0);
}

function renderProbPanel() {
  const act = prizes.filter(p => p.active);
  const total = activeTotal();
  document.getElementById('probTotal').textContent = fmt(total) + '%';
  ...
  if (total > 0) {
    act.forEach(p => {
      ...
      seg.style.width = (pct / total * 100) + '%';
      seg.title = `${p.name}: ${fmt(pct)}%${Math.abs(total - 100) > 0.01 ? ` (реально ≈${fmt(pct / total * 100)}%)` : ''}`;
      bar.appendChild(seg);
    });
  }
  ...
  if (Math.abs(total - 100) < 0.01) {
    note.textContent = '✓ Сумма = 100%. Реальные шансы совпадают с указанными.';
    normBtn.style.display = 'none';
  } else {
    note.textContent = total === 0
      ? 'У всех активных призов вероятность 0%.'
      : 'Сумма ≠ 100% — реальные шансы считаются пропорционально. Нажмите «Уравнять», чтобы проценты стали точными.';
    normBtn.style.display = '';
  }
}

document.getElementById('normalizeBtn').onclick = async () => {
  const act = prizes.filter(p => p.active);
  const total = activeTotal();
  if (!total) return;

  const rounded = act.map(p => Math.round(((Number(p.percent) || 0) / total) * 1000) / 10);
  ...
  try {
    for (let i = 0; i < act.length; i++) {
      ...
      fd.append('percent', rounded[i]);
      await api('/api/admin/prizes/' + act[i].id, { method: 'PUT', body: fd });
    }
    await loadPrizes();
...
function renderCards() {
  ...
  const total = activeTotal();
  const uneven = Math.abs(total - 100) > 0.01;

  prizes.forEach(p => {
    const pct = Number(p.percent) || 0;
    let chance;
    if (pct === 0) chance = '🎯 0% (не выпадает)';
    else {
      chance = `🎯 ${fmt(pct)}%`;
      if (p.active && uneven && total > 0)
        chance += ` <small>(реально ≈${fmt(pct / total * 100)}%)</small>`;
    }
    ...
    el.innerHTML = `
      ...
      <div class="card__info">
        ...
        <div class="card__meta">${chance} • 📦 ${p.stock ? 'осталось ' + p.stock : '∞'}</div>
      </div>
...
function openForm(p) {
  ...
  form.reset();
  form.percent.value = p ? p.percent : 10;
